...
//        float **tmp=NULL;
//        tmp=(float**)malloc(crd_size*sizeof(float*));
//        for(i=0;i<crd_size;i++)
//        {
//             tmp[i]=(float*)malloc(3*sizeof(float));
//             memcpy(tmp[i],ref[i],3*sizeof(float));
//        }

//        center_sys2(natom,tmp,crd_size,index_c,index_r);
//        center_sys2(natom,crd,crd_size,index_c,index_r);
...
//  printf("from getcentroid k is %d \n",k);
...
void center_sys(int natom, float **crd, int crd_size, int index_c[], int index_r[])
{
  ...
  //printf("centroid is %lf %lf %lf \n",c.X,c.Y,c.Z);

  for (i=0 ; i < crd_size ; i++)
  {
        //printf("step %d : Before \t %f \t %f \t %f \t",i,crd[i][0],crd[i][1],crd[i][2]);
        crd[i][0] -= c.X;
        crd[i][1] -= c.Y;
        crd[i][2] -= c.Z;
